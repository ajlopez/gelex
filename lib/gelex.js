    ...
    this.define = function (name, data) {
        if (Array.isArray(data))
            for (let k = 0; k < data.length; k++)
                addRule(name, createRule(data[k]));
        else
            addRule(name, createRule(data));
    };
    
    this.defineText = function (name, start, end) {
        addRule(name, new TextRule(start, end));
    }
